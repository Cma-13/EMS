  ...
  try {
    ...
    try {
      const faceRes = await fetch(
        `${FACE_SERVICE_URL}/delete-face/${employee.employeeId}`,
        {
          method: "DELETE",
      ...
      if (!faceRes.ok && faceRes.status !== 404) {
        console.warn(
          `Face embedding delete returned ${faceRes.status} for ${employee.employeeId}`,
        );
      }
    } catch (faceErr) {
      console.warn(
        `Could not reach face service to delete embedding for ${employee.employeeId}:`,
        faceErr.message,
      );
    }
    ...
    await Promise.all([
      ...
      LeaveBalance.deleteMany({ employeeId: employee._id }),
      Notification.deleteMany({ forUserId: employee.userId }),
    ]);
