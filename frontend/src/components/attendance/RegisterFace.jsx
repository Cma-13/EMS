...
const RegisterFace = () => {
  ...
  const capturePhoto = () => {
    ...
    setMessage(
      ` ${captures.length + 1} photo(s) captured. ${captures.length < 2 ? "Capture more for better accuracy." : "You can register now."}`,
    );
  };
  ...
  const registerFace = async () => {
    ...
    if (captures.length === 0) {
      setMessage("Please capture at least one photo.");
      return;
    }
    setStatus("loading");
    ...
    try {
      const res = await axios.post(
        ...
        { employeeId: selectedEmployee, images: captures },
        { headers: faceHeaders() },
      );
      if (res.data.success) {
        setStatus("success");
        ...
        setStatus("error");
        setMessage(
          ` ${res.data.error || "Registration failed. No clear face detected."}`,
        );
      }
    } catch (err) {
      setStatus("error");
      if (err.response) {
        // Face service responded with an error
        const errorMsg =
          err.response.data?.error ||
          err.response.data?.message ||
          "Registration failed.";
        // Specific error for duplicate face
        ...
        // Face service not running
        setMessage(
          " Cannot connect to face recognition service. Make sure the Python face service is running on port 5001.",
        );
      } else {
        setMessage(
    ...
    <div className="p-6 bg-stone-50 min-h-full">
      <div className="max-w-xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-stone-200 p-6">
          ...
          <div
            className="bg-stone-900 rounded-xl overflow-hidden mb-4 relative"
            style={{ minHeight: 240 }}
          >
            <video
              ref={videoRef}
              ...
              {captures.map((img, i) => (
                <div key={i} className="relative">
                  <img
                    src={img}
                    alt=""
                    className="w-14 h-14 object-cover rounded-lg border-2 border-teal-400"
                  />
                  <span className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-teal-500 text-white text-xs rounded-full flex items-center justify-center font-bold">
                    {i + 1}
                  </span>
                </div>
              ))}
          ...
          <div className="flex gap-2 mb-4">
            {!stream ? (
              <button
                onClick={startCamera}
                className="flex-1 flex items-center justify-center gap-2 bg-teal-600 hover:bg-teal-700 text-white font-semibold py-3 rounded-xl text-sm transition"
              >
              ...
              <>
                ...
                <button
                  ...
                  disabled={status === "loading" || captures.length === 0}
